...
from ._v_schedule import BINS_PER_DAY, DT_BIN_HOURS
...
_OBS_PARAMS = dict(
    ...
    # delta_HR is the subject-specific baseline offset; default 0.0
    # = population-baseline subject. Per LaTeX §3.1, HR mean is
    # HR_base + delta_HR + alpha_HR * W. Synthesis uses the value
    # set here (or whatever the test harness overrides it with).
    HR_base=50.0,
    delta_HR=0.0,
    alpha_HR=25.0,
    sigma_HR=8.0,
    ...
    # delta_s is the subject-specific baseline offset; default 0.0
    # = population-baseline subject. Per LaTeX §3.4, stress mean is
    # s_base + delta_s + alpha_s * W + beta_s * V_n.
    s_base=30.0,
    delta_s=0.0,
...
def scenario_presets(t_total_days: int) -> dict:
    ...
    Mirrors the canonical PARAM_SET_*/INIT_STATE_* values from
    Python-Model-Development-Simulation. All scenarios use constant
    daily controls — the bench tool will perturb V_c (and V_h, V_n)
    under closed-loop control. These constant-truth schedules are
    used by the 14-day plant regression scenarios in
    `swat_model_factory/scenarios/`.
    """
...
def gen_obs_hr(trajectory, t_grid_days, params, seed=42):
    """Gaussian HR channel: hr ~ N(HR_base + delta_HR + alpha_HR * W, sigma_HR^2).
    ...
    hr_mean = (params['HR_base'] + params['delta_HR']
               + params['alpha_HR'] * W)
...
def gen_obs_stress(trajectory, t_grid_days, params, V_n_per_bin,
    ...
    stress ~ N(s_base + delta_s + alpha_s * W + beta_s * V_n, sigma_s^2),
    clipped to [0, 100].
...
def forward_sim_set(scenario_name: str, t_total_days: int = 14,
    ...
    Used as a quick deterministic smoke check (no plant noise) for
    ...
    from ._dynamics import drift_jax
    from ._v_schedule import expand_three_schedules
